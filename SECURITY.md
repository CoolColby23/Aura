...
In-app update archives are published through the official GitHub release workflow and authenticated with Sparkle EdDSA signatures. The private update key is stored as an encrypted repository Actions secret and must never be committed, logged, or attached to a release; only its public key is embedded in Aura.
