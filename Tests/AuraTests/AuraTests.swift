...
struct PlaybackSessionTests {
    let track = TrackMetadata(
        identity: TrackIdentity(persistentID: "1"), title: "Track", artist: "Artist", album: "Album", duration: 100, source: .appleMusicCatalog,
    ...
    @Test func eligibleTrackIsFinalizedAsPlayed() async {
        ...
        let nextTrack = TrackMetadata(
            identity: TrackIdentity(persistentID: "2"), title: "Next", artist: "Artist", album: nil, duration: 100, source: .appleMusicCatalog,
            appleMusicURL: nil, artworkReference: nil)
        _ = await tracker.ingest(.init(track: track, state: .playing, position: 0, observedAt: start, confidence: .high))
        _ = await tracker.ingest(.init(track: track, state: .playing, position: 50, observedAt: start.addingTimeInterval(50), confidence: .high))
    ...
    @Test func eligibleTrackRemainsActiveUntilPlaybackStops() async {
        ...
        let thresholdEvents = await tracker.ingest(
            .init(track: track, state: .playing, position: 50, observedAt: start.addingTimeInterval(50), confidence: .high))
    ...
    @Test func shortTrackIsNotScrobbleable() {
        let short = TrackMetadata(
            identity: .init(persistentID: "2"), title: "Tiny", artist: "Artist", album: nil, duration: 30, source: .localFile, appleMusicURL: nil,
    ...
    @Test func playingTrackReplacementBeforeThresholdIsSkipped() async {
        let tracker = PlaybackSessionTracker()
        let start = Date(timeIntervalSince1970: 1_000)
        let next = TrackMetadata(
            identity: .init(persistentID: "next"), title: "Next", artist: "Artist", album: nil, duration: 100, source: .appleMusicCatalog, appleMusicURL: nil,
            artworkReference: nil)
        _ = await tracker.ingest(.init(track: track, state: .playing, position: 5, observedAt: start, confidence: .high))
    ...
    @Test func unsupportedLiveStreamIsNeverEligible() async {
        let stream = TrackMetadata(
            identity: .init(persistentID: "live"), title: "Station", artist: "Host", album: nil, duration: 3_600, source: .radioStream, appleMusicURL: nil,
    ...
    @Test func appleMusicRadioQualifiesAfterObservedListeningAndFinalizesPlayed() async {
        let tracker = PlaybackSessionTracker()
        let start = Date(timeIntervalSince1970: 1_000)
        let radio = TrackMetadata(
            identity: .init(persistentID: "radio:one"), title: "One", artist: "DJ", album: "Station", duration: 0, source: .radioStream, appleMusicURL: nil,
            artworkReference: nil)
        let next = TrackMetadata(
            identity: .init(persistentID: "radio:two"), title: "Two", artist: "DJ", album: "Station", duration: 0, source: .radioStream, appleMusicURL: nil,
            artworkReference: nil)
    ...
    @Test func listeningPresentationReportsProgressAndRemainingTime() {
        var session = PlaybackSession(
            id: UUID(), track: track, startedAt: .now, accumulatedPlayTime: 25, lastPosition: 25, eligibility: .listening, outcome: .active)
    ...
    @Test func ineligiblePresentationExplainsStreams() {
        ...
        let session = PlaybackSession(
            id: UUID(), track: stream, startedAt: .now, accumulatedPlayTime: 0, lastPosition: 0, eligibility: .ineligible, outcome: .active)
...
struct AdditionalPlaybackPlatformTests {
    ...
    @Test func parsesSpotifyPlayingAndPausedStates() throws {
        let playing = ["playing", "Track", "Artist", "Album", "180000", "42.5", "spotify:track:abc", "spotify:track:abc", "https://i.scdn.co/image/cover"]
    ...
    @Test func sourceSelectionPrefersPlayingAndRecoversAfterProviderLoss() throws {
        ...
        let youtubeData = Data(
            #"{"player":{"trackState":1,"videoProgress":4},"video":{"author":"YT Artist","title":"YT Track","durationSeconds":200,"id":"yt","isLive":false}}"#
                .utf8)
    ...
    @Test func parsesYTMDesktopCompanionState() throws {
        let data = Data(
            #"{"player":{"trackState":1,"videoProgress":42.5},"video":{"author":"Artist","title":"Track","album":"Album","durationSeconds":180,"id":"abc123","isLive":false,"thumbnails":[{"url":"https://example.com/small.jpg"},{"url":"https://example.com/large.jpg"}]}}"#
                .utf8)
        let observedAt = Date(timeIntervalSince1970: 1_000)
    ...
    @Test func liveYTMDesktopVideoIsNotScrobbleable() throws {
        let data = Data(
            #"{"player":{"trackState":1,"videoProgress":4},"video":{"author":"Station","title":"Live","durationSeconds":3600,"id":"live","isLive":true}}"#.utf8)
...
struct SecurityTests {
    @Test func encryptedBackupRoundTripsAndRejectsWrongPassphrase() throws {
        ...
        #expect(
            try SecureBackupService.decrypt(
                encrypted,
                passphrase: "correct horse battery staple"
            ) == original)
    ...
    @Test func credentialsMigrateFromTheRenamedPresenceFMDirectory() async throws {
        // The app shipped as PresenceFM through v1.3, so an Aura build must adopt
        // that credential file rather than start the user disconnected.
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let legacyDirectory = root.appendingPathComponent("PresenceFM", isDirectory: true)
        try FileManager.default.createDirectory(at: legacyDirectory, withIntermediateDirectories: true)
        try Data(#"{"lastFMSessionKey":"legacy-session"}"#.utf8)
            .write(to: legacyDirectory.appendingPathComponent("credentials.json"))

        let auraDirectory = root.appendingPathComponent("Aura", isDirectory: true)
        let adopted = CredentialStore(baseDirectory: auraDirectory)
        #expect(await adopted.value(for: .lastFMSessionKey) == "legacy-session")

        // The adopted values are written forward, so the legacy file is only read once.
        try FileManager.default.removeItem(at: legacyDirectory)
        let reloaded = CredentialStore(baseDirectory: auraDirectory)
        #expect(await reloaded.value(for: .lastFMSessionKey) == "legacy-session")
    }

    @Test func credentialsPreferThisBuildsFileOverTheLegacyOne() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let legacyDirectory = root.appendingPathComponent("PresenceFM", isDirectory: true)
        try FileManager.default.createDirectory(at: legacyDirectory, withIntermediateDirectories: true)
        try Data(#"{"lastFMSessionKey":"stale-session"}"#.utf8)
            .write(to: legacyDirectory.appendingPathComponent("credentials.json"))

        let auraDirectory = root.appendingPathComponent("Aura", isDirectory: true)
        let store = CredentialStore(baseDirectory: auraDirectory)
        try await store.set("current-session", for: .lastFMSessionKey)

        let reloaded = CredentialStore(baseDirectory: auraDirectory)
        #expect(await reloaded.value(for: .lastFMSessionKey) == "current-session")
    }
...
struct PersistenceAndQueueTests {
    private func session(id: UUID = UUID()) -> PlaybackSession {
        let track = TrackMetadata(
            identity: .init(persistentID: "persisted"), title: "Track", artist: "Artist", album: "Album", duration: 100, source: .appleMusicCatalog,
            appleMusicURL: nil, artworkReference: nil)
        return PlaybackSession(
            id: id, track: track, startedAt: Date(timeIntervalSince1970: 1_000), accumulatedPlayTime: 50, lastPosition: 50, eligibility: .eligible,
            outcome: .queued)
    }
    ...
    @Test func queueReportsRejectedAdmissionWithoutPersistingThePlay() throws {
        ...
        #expect(
            queue.enqueue(distinctSession(title: "First", minutesAfterEpoch: 10))
                == .warning(
                    "The offline scrobble queue is approaching its 5,000-item limit."
                ))
        #expect(
            queue.enqueue(distinctSession(title: "Rejected", minutesAfterEpoch: 11))
                == .rejected(
...
struct PlaybackCoordinationTests {
    @Test func playbackProgressAdvancesLocallyClampsAndThrottlesAnnouncements() {
        ...
        #expect(
            PlaybackProgressPresentation.position(
                for: playing,
...
struct BackupAndExtendedInsightsTests {
    ...
    @Test func csvV1HasFrozenColumnsAndUnicode() throws {
        ...
        #expect(
            csv.hasPrefix("schema_version,id,started_at,finalized_at,title,artist,album,platform,outcome,duration_seconds,listening_seconds,persistent_id\n"))
...
struct PersistenceRecoveryTests {
    ...
    @Test func migratesVersionZeroFixtureAndPreservesOlderOptionalData() throws {
        ...
        let url = root.appendingPathComponent("Aura.store")

        try autoreleasepool {
            let schema = Schema(versionedSchema: AuraSchemaV0.self)
            let configuration = ModelConfiguration("Aura-v0", schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: configuration)
            let context = container.mainContext
            context.insert(
                AuraSchemaV0.ActivityRecord(
    ...
    @Test func importsLegacyStoreBacksUpAndRestoresWithoutDeletingOriginal() throws {
        ...
        let store = root.appendingPathComponent("Aura/Aura.store")
    ...
    @Test func onlyTwoPhysicalMigrationBackupsAreRetained() throws {
        ...
        defer { try? FileManager.default.removeItem(at: root) }
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let store = root.appendingPathComponent("Aura.store")
