...
struct AuraCoreTests {
    ...
    @Test func reconciledHistoryRequiresExplicitImportSelection() {
        ...
        #expect(
            ScrobbleEligibilityPolicy.evaluate(
                historical,
    ...
    @Test func rescanningTheSameReconciledPlayDoesNotDuplicateEvidence() {
        let played = Date(timeIntervalSince1970: 2_000)
        let first = PlaybackEvidence(
            deviceID: deviceID, sourceTrackID: "history-song",
            metadata: .init(title: "Track", artist: "Artist", startedAt: played),
            observedPlayTime: nil, origin: .reconciled, confidence: .probable)
        let duplicate = PlaybackEvidence(
            deviceID: deviceID, sourceTrackID: "history-song",
            metadata: .init(title: "Track", artist: "Artist", startedAt: played),
            observedPlayTime: nil, origin: .reconciled, confidence: .probable)
        let listen = CanonicalListen(
            id: CanonicalListenIdentity.make(for: first), evidence: [first],
            metadata: first.originalMetadata, state: .review)
    ...
    @Test func accessibilitySummaryReadsAsOneStatement() {
        ...
        #expect(
            progressing.accessibilitySummary == """
