...
Search existing GitHub issues before opening a new one. Include the Aura version, macOS version, affected integration, reproduction steps, and the redacted Diagnostics view. Never include Last.fm tokens, API secrets, or unredacted home-directory paths.
...
Before removing Aura, turn off **Launch Aura at login** in Settings.
Quit the app, then move Aura from Applications to the Trash.
...
data too, delete `~/Library/Application Support/Aura` and run
`defaults delete fm.aura.Aura` in Terminal. This is permanent; export
a Aura backup first if the history or queue should be preserved.
