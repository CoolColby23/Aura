# Aura 1.0.0 Release-Candidate QA
...
- [x] Pull-request CI at head `ead5201` ([run 30727734145](https://github.com/CoolColby23/Aura/actions/runs/30727734145)) passes with pinned Xcode 26.0: 97 Swift tests plus coverage, patch hygiene, strict formatting of the new files, website verification, app packaging, bundle verification, DMG creation, and `hdiutil verify`.
...
- [x] Local cleanup removed all 76 records identified by the reserved `aura-demo-` persistent-ID prefix, removed no queued scrobbles, preserved all 418 then-current real history records, and left the SQLite store with a successful integrity check. The pre-cleanup store is recoverable from the app's manual backup directory. Candidate `ead5201` now performs this scoped cleanup whenever Demo Mode ends.
