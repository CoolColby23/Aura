...
enum PersistenceRecovery {
    ...
    static func defaultStoreURL(fileManager: FileManager = .default) throws -> URL {
        try applicationDirectory(fileManager: fileManager).appendingPathComponent("Aura.store")
    }

    /// The store a fresh Aura install imports from, if one is present.
    ///
    /// The app shipped as PresenceFM through v1.3, so that store is preferred over
    /// the older flat `default.store`. `prepare(storeURL:legacyStoreURL:)` copies it
    /// forward once and then writes a sentinel, so this only runs on first launch
    /// after the rebrand.
    static func legacyStoreURL(fileManager: FileManager = .default) throws -> URL {
        ...
        let presenceFM =
            ...
            .appendingPathComponent("PresenceFM.store")
        if fileManager.fileExists(atPath: presenceFM.path) { return presenceFM }
