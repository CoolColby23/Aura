...
enum AuraSchemaV0: VersionedSchema {
...
typealias ActivityRecord = AuraSchemaV1.ActivityRecord
typealias ScrobbleRecord = AuraSchemaV1.ScrobbleRecord
typealias DiagnosticRecord = AuraSchemaV1.DiagnosticRecord
typealias IntegrationHealthEvent = AuraSchemaV1.IntegrationHealthEvent

enum AuraMigrationPlan: SchemaMigrationPlan {
    static var schemas: [any VersionedSchema.Type] { [AuraSchemaV0.self, AuraSchemaV1.self] }
    static var stages: [MigrationStage] {
        [.lightweight(fromVersion: AuraSchemaV0.self, toVersion: AuraSchemaV1.self)]
    }
}
...
final class PersistenceStore {
    ...
    init(
        ...
        if inMemory {
            ...
            configuration = ModelConfiguration("Aura", schema: schema, url: resolved)
        }
    ...
    func recordHealth(_ integration: IntegrationID, state: IntegrationState, at timestamp: Date = .now) {
        ...
        if let latest = try? context.fetch(descriptor).first,
            latest.stateRaw == state.rawValue
        {
            return
        }
