...
final class AppModel {
    ...
    init(
        ...
        privateModeIntentObserver = NotificationCenter.default.addObserver(
            forName: .auraPrivateModeIntent,
            object: nil,
            queue: .main
        ) { [weak self] notification in
            guard let rawAction = notification.userInfo?["action"] as? String,
                let action = PrivateModeIntentAction(rawValue: rawAction)
            else { return }
            Task { @MainActor in
                switch action {
    ...
    func saveVerificationReport() {
        Task {
            do {
                ...
                panel.nameFieldStringValue = "Aura-Verification-\(ReleaseConfiguration.version).json"
    ...
    func makeVerificationReport(now: Date = .now) async throws -> VerificationReport {
        ...
        let queued = try store.context.fetchCount(
            FetchDescriptor<ScrobbleRecord>(
                predicate: #Predicate { $0.stateRaw == pending || $0.stateRaw == retrying }
            ))
        let failed = try store.context.fetchCount(
            FetchDescriptor<ScrobbleRecord>(
                predicate: #Predicate { $0.stateRaw == permanentlyFailed }
            ))
        let cache = await artwork.cacheMetrics
        let statuses = Dictionary(
            uniqueKeysWithValues: integrationHealth.map {
        ...
        return VerificationReport(
            ...
            polling: .init(
                ...
                providerMilliseconds: Dictionary(
                    uniqueKeysWithValues: playbackPollMetrics.providerDurations.map {
    ...
    func restoreLatestDatabaseBackup() {
        ...
        do {
            ...
            persistenceRecoveryStatus = "The latest automatic database backup was restored. Restart Aura to open it."
    ...
    func prepareFreshDatabase() {
        ...
        do {
            ...
            persistenceRecoveryStatus = "The failed database was preserved. Restart Aura to create a new local store."
        } catch { persistenceRecoveryStatus = error.localizedDescription }
    }
    ...
    func restartApplication() {
        ...
        process.arguments = ["-c", "sleep 1; /usr/bin/open -n \"$1\"", "aura-restart", Bundle.main.bundlePath]
        do {
            try process.run()
            shutdown()
            NSApp.terminate(nil)
        } catch { persistenceRecoveryStatus = "Quit and reopen Aura to retry. \(error.localizedDescription)" }
    }
    ...
    var captureStatus: CaptureStatusPresentation {
        ...
        if musicStatus == .awaitingPermission {
            return CaptureStatusPresentation(
                ...
                explanation: "Allow music-player access so Aura can detect what is playing.",
        ...
        recentSessions.prefix(3).map { session in
            ...
            case .queued, .failed:
                CaptureStatusPresentation(
                    status: .queued, headline: session.track.title, explanation: "Saved locally for retry", timestamp: session.startedAt,
                    recoveryAction: .retryQueue)
            case .played:
                CaptureStatusPresentation(
                    status: .excluded, headline: session.track.title, explanation: "Played, but not sent to Last.fm", timestamp: session.startedAt)
            case .skipped, .interrupted:
                CaptureStatusPresentation(
                    status: .excluded, headline: session.track.title, explanation: "Not enough listening time to scrobble", timestamp: session.startedAt)
            case .active, .listened:
                CaptureStatusPresentation(
                    status: .progressing, headline: session.track.title, explanation: "Listening activity detected", timestamp: session.startedAt)
            }
        }
    }
    ...
    private func updateOperationalNotifications() async {
        if musicStatus == .awaitingPermission {
            await notifications.notifyOnce(
                ...
                body: "Open Aura to restore Automation permission.",
                section: .diagnostics
            )
        } else {
        ...
        if lastFMStatus == .authorizationExpired {
            await notifications.notifyOnce(
                ...
                body: "Your Last.fm authorization expired. Open Aura to reconnect.",
                section: .settings
            )
        } else {
...
final class Preferences {
    ...
    init(defaults: UserDefaults = .standard, cloudStore: NSUbiquitousKeyValueStore? = nil) {
        ...
        discordActivityName = defaults.string(forKey: "discordActivityName") ?? "Aura"
        discordTimerStyle =
        ...
        discordPresenceProfiles =
            defaults.data(forKey: "discordPresenceProfiles")
        ...
        if let cloudStore {
            ...
            // that have already synced from another Aura installation.
