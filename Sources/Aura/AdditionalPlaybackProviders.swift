...
enum YTMDesktopError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unauthorized: "Reconnect YTMDesktop in Aura settings."
        }
    }
}
...
actor YTMDesktopClient {
    ...
    func authorize() async throws -> String {
        let codeResponse = try await post(
            "auth/requestcode",
            body: ["appId": "aura", "appName": "Aura", "appVersion": ReleaseConfiguration.version]
        )
        guard let code = codeResponse["code"] as? String else { throw YTMDesktopError.invalidResponse }
        let tokenResponse = try await post("auth/request", body: ["appId": "aura", "code": code], timeout: 35)
    ...
    nonisolated static func parseSnapshot(_ data: Data, observedAt: Date = .now) throws -> PlaybackSnapshot? {
        ...
        else { throw YTMDesktopError.invalidResponse }
    ...
    private func post(_ path: String, body: [String: String], timeout: TimeInterval = 4) async throws -> [String: Any] {
        ...
        do {
            ...
            return object
        } catch let error as YTMDesktopError { throw error } catch { throw YTMDesktopError.unavailable }
    }
}
...
/// This adapter dynamically loads that service so Aura still launches if
...
enum TidalNowPlayingProvider {
    ...
    static func snapshot() async -> PlaybackSnapshot? {
        ...
        guard let app = NSRunningApplication(processIdentifier: pid),
            app.bundleIdentifier?.localizedCaseInsensitiveContains("tidal") == true
        else { return nil }
        ...
        let track = TrackMetadata(
            ...
            appleMusicURL: URL(
                string: "https://listen.tidal.com/search?q=\("\(artist) \(title)".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")"),
