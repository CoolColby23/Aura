...
actor PlaybackMonitor: PlaybackProviding {
    ...
    private let performanceLog = OSLog(subsystem: "fm.aura.Aura", category: "PlaybackPolling")
    ...
    private func readPlayback() async -> PlaybackMonitorUpdate {
        ...
        let refreshesAllHealth =
            clock.now.timeIntervalSince(lastFullHealthRefresh)
    ...
    nonisolated static func parseMusic(_ value: String, observedAt: Date = .now) -> PlaybackSnapshot {
        ...
        return PlaybackSnapshot(
            track: track, state: state, position: position, observedAt: observedAt,
