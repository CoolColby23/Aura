...
struct AppTheme: Identifiable, Hashable {
    ...
    var primaryColor: Color { Color(hex: primaryHex) ?? BrandColors.indigo }
    var secondaryColor: Color { Color(hex: secondaryHex) ?? BrandColors.lilac }
    var darkBackground: Color { Color(hex: darkBackgroundHex) ?? BrandColors.dusk }
    var lightBackground: Color { Color(hex: lightBackgroundHex) ?? BrandColors.haze }
    ...
    static let defaultID = "aura"
    static let presets: [AppTheme] = [
        .init(
            id: "aura", name: "Aura", description: "Peach through indigo", primaryHex: "#5B5BF5", secondaryHex: "#A78BFA",
            darkBackgroundHex: "#0F0F17", lightBackgroundHex: "#F7F6FB"),
        .init(
