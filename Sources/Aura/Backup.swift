...
extension UTType {
    static let auraBackup = UTType(exportedAs: "fm.aura.backup", conformingTo: .json)
    /// Backups written by PresenceFM builds. Imported so existing backup files stay
    /// restorable after the rebrand; Aura never writes this type.
    static let legacyPresenceFMBackup = UTType(importedAs: "fm.presence.backup", conformingTo: .json)
}

struct AuraBackup: Codable {
...
struct AuraBackupDocument: FileDocument {
    static var readableContentTypes: [UTType] { [.auraBackup, .legacyPresenceFMBackup, .json] }
    let backup: AuraBackup

    init(backup: AuraBackup) { self.backup = backup }
...
enum BackupError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidDocument: "This is not a valid Aura backup."
        case .unsupportedVersion(let version): "Backup format \(version) is newer than this version of Aura supports."
...
enum BackupService {
    ...
    nonisolated static func encode(_ backup: AuraBackup) throws -> Data {
        try makeEncoder().encode(backup)
    }

    nonisolated static func decode(_ data: Data) throws -> AuraBackup {
        try makeDecoder().decode(AuraBackup.self, from: data)
    }

    static func make(store: PersistenceStore, preferences: Preferences, now: Date = .now) throws -> AuraBackup {
        ...
        return AuraBackup(
            formatVersion: AuraBackup.currentFormatVersion, createdAt: now,
            appVersion: ReleaseConfiguration.version, activity: activity, queue: queue,
            preferences: snapshot(preferences)
        )
    }

    nonisolated static func validate(_ backup: AuraBackup) throws {
        guard backup.formatVersion <= AuraBackup.currentFormatVersion else {
    ...
    static func restore(_ backup: AuraBackup, store: PersistenceStore, preferences: Preferences) throws {
        try validate(backup)
        if !store.isInMemory {
    ...
    private static func writeAutomaticBackup(_ backup: AuraBackup) throws {
        let base = try FileManager.default.url(
            for: .applicationSupportDirectory, in: .userDomainMask,
            appropriateFor: nil, create: true
        ).appendingPathComponent("Aura/Backups", isDirectory: true)
        try FileManager.default.createDirectory(at: base, withIntermediateDirectories: true)
        let name = "automatic-\(Int(backup.createdAt.timeIntervalSince1970)).aurabackup"
        ...
        ).filter { $0.pathExtension == "aurabackup" }.sorted {
    ...
    private static func apply(_ value: PreferenceBackup, to preferences: Preferences) {
        ...
        preferences.discordActivityName = value.discordActivityName ?? "Aura"
        preferences.discordTimerStyle =
            DiscordTimerStyle(rawValue: value.discordTimerStyle ?? "")
